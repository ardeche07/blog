...
description: Learn best practices to build and deploy a security-hardened SSH bastion host based on OpenSSH server.
...
This blog post focuses on following best practices on building and deploying a secure SSH bastion server based on OpenSSH. If you are looking for a quick guide on creating an SSH bastion, check our previous blog post on [setting up an SSH Bastion](https://goteleport.com/blog/ssh-bastion-host/).
...
If you prefer simplicity, you can also use [UFW](https://help.ubuntu.com/community/UFW), which vastly simplifies iptables management. Another handy Linux tool to view open network ports is `lsof`. `lsof` lists the process ID of a service that is listening on a particular port. For example, to list open TCP ports that are in a listening state, use the following `lsof` command:
