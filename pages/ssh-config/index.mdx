...
Elegant and simple. Now that we have the basics, let’s see what’s actually going on here. My choice of ed25519 is explained in [Comparing SSH Keys - RSA, DSA, ECDSA, or EdDSA?](/comparing-ssh-keys/)
...
An [ssh jump server](/ssh-jump-server/) is a proxy standing between clients and the rest of the `ssh` fleet. Jump hosts minimize threats by forcing all `ssh` traffic to go through a single hardened location and minimizing an individual node’s `ssh` endpoints to the outside world.
...
<BlogImage
  src="./assets/ssh-config-1.png"
  alt="diagram of ssh with multiple jumps"
  width="100%"
/>
